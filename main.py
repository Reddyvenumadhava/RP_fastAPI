...
async def health_check():
    ...
    try:
        with SessionLocal() as db:
            # NaukriDB candidate count for health check
            result = db.execute(text("SELECT COUNT(*) FROM tbl_cd_candidates"))
            student_count = result.scalar()
            
            # Resume hash registry count for duplicate detection
            result2 = db.execute(text("SELECT COUNT(*) FROM tbl_cd_resume_hashes"))
