...
def _is_missing_resume_hash_table_error(err: Exception) -> bool:
	...
	return (
		...
		or "42s02" in err_text
			or "invalid object name 'tbl_cd_resume_hashes'" in err_text
...
def _ensure_resume_hash_table(db: Session) -> None:
	...
	try:
		db.execute(
			text(
				"""
					CREATE TABLE IF NOT EXISTS tbl_cd_resume_hashes (
		...
		db.commit()
			...
			logger.warning(f"Could not create tbl_cd_resume_hashes: {e}")
...
def _find_existing_resume_hash(db: Session, resume_hash: str):
	...
	try:
		result = _execute_with_db_retry(
			db,
			text("SELECT student_id FROM tbl_cd_resume_hashes WHERE hash = :hash"),
			{"hash": resume_hash},
		)
		...
		if not _is_missing_resume_hash_table_error(e):
			raise
		db.rollback()
		logger.warning("tbl_cd_resume_hashes missing. Creating table and retrying hash lookup.")
...
def _save_resume_hash(db: Session, resume_hash: str, student_id: int) -> None:
	...
	try:
		db.execute(
			text("INSERT INTO tbl_cd_resume_hashes (hash, student_id) VALUES (:hash, :student_id)"),
		...
		if not _is_missing_resume_hash_table_error(e):
			raise
		db.rollback()
		logger.warning("tbl_cd_resume_hashes missing during insert. Creating table and retrying.")
		_ensure_resume_hash_table(db)
		db.execute(
			text("INSERT INTO tbl_cd_resume_hashes (hash, student_id) VALUES (:hash, :student_id)"),
			{"hash": resume_hash, "student_id": student_id},
...
def run_full_pipeline(db: Session, file_bytes: bytes, filename: str) -> Dict[str, Any]:
	...
	try:
		...
		student_id = student_result.candidate_id
	...
	# STEP 6 — Save education records
	logger.info("STEP 6 — Save education records")
	try:
			...
			try:
				save_service.save_education(
					...
					SaveEducationRequest(
						student_id=student_id,
						institution_name=edu_item.college_name,
						degree=edu_item.course_name,
						field_of_study=edu_item.specialization_name or "General",
						start_date=f"{edu_item.start_year}-01-01" if edu_item.start_year else None,
						end_date=f"{edu_item.end_year}-01-01" if edu_item.end_year else None,
						is_pursuing=getattr(edu_item, "is_pursuing", False),
						grading_system=getattr(edu_item, "grading_system", None),
						score_obtained=edu_item.cgpa if edu_item.cgpa is not None else edu_item.percentage,
						maximum_score=10.0 if edu_item.cgpa is not None else 100.0,
					),
				)
	...
	# STEP 10 — Languages are not modeled in NaukriDB
	logger.info("STEP 10 — Languages skipped (not modeled in NaukriDB)")
	...
	# STEP 12 — Interests are not modeled in NaukriDB
	logger.info("STEP 12 — Interests skipped (not modeled in NaukriDB)")

	# STEP 13 — Addresses are not modeled in NaukriDB
	logger.info("STEP 13 — Addresses skipped (not modeled in NaukriDB)")
...
def save_confirmed_resume(db: Session, resume_hash: str, parsed: ParsedResume) -> Dict[str, Any]:
	...
	# STEP 10 — Languages are not modeled in NaukriDB
	logger.info("Save-confirmed step 10 skipped (languages not modeled in NaukriDB)")
	...
	# STEP 12 — Interests are not modeled in NaukriDB
	logger.info("Save-confirmed step 12 skipped (interests not modeled in NaukriDB)")

	# STEP 13 — Addresses are not modeled in NaukriDB
	logger.info("Save-confirmed step 13 skipped (addresses not modeled in NaukriDB)")
